...
//      mCosts = aCosts;
	...
	public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte aSide, float hitX, float hitY, float hitZ) {
		...
		if (aTileEntity instanceof IGregTech6TileEntity) {
			IMetaTileEntity tMetaTileEntity = ((IGregTech6TileEntity)aTileEntity).getMetaTileEntity();
					...
					for (int i = 0, j = tTileEntity.getSizeInventory(); i < j; i++) if (tTileEntity.isValidSlot(i)) {
						if (tTileEntity.getStackInSlot(i) != null) {
							if (aPlayer.capabilities.isCreativeMode || ((ItemMetaTool)aItem).doDamage(aStack, mCosts)) {
								...
								if (tStack != null) {
									EntityItem tEntity = new EntityItem(aWorld, ((IGregTech6TileEntity)aTileEntity).getOffsetX((byte)aSide, 1) + 0.5, ((IGregTech6TileEntity)aTileEntity).getOffsetY((byte)aSide, 1) + 0.5, ((IGregTech6TileEntity)aTileEntity).getOffsetZ((byte)aSide, 1) + 0.5, tStack);
									tEntity.motionX = 0; tEntity.motionY = 0; tEntity.motionZ = 0;
									aWorld.spawnEntityInWorld(tEntity);
									UT.Sounds.send(aWorld, GregTech6_API.sSoundList.get(101), 1.0F, -1, aX, aY, aZ);
								}
								return T;
							}
						}
