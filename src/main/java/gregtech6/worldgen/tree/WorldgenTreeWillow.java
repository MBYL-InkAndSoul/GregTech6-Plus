 ...
 */
public class WorldgenTreeWillow extends WorldgenOnSurface {
	@SafeVarargs
	public WorldgenTreeWillow(String aName, boolean aDefault, int aAmount, int aProbability, List<WorldgenObject>... aLists) {
	...
	public int canGenerate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ, Random aRandom, BiomeGenBase[][] aBiomes, Set<String> aBiomeNames) {
		...
		for (String tName : aBiomeNames) if (BIOMES_WILLOW.contains(tName)) return mAmount;
		return 0;
	}
	...
	public boolean tryPlaceStuff(World aWorld, int aX, int aY, int aZ, Random aRandom, Block aContact) {
		...
		return ((BlockBaseSapling)BlocksGT.Saplings_AB).grow(aWorld, aX, aY+1, aZ, (byte)2, aRandom);
	}
}
