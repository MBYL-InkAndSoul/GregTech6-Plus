...
package gregtech6.asm.transformers;
		...
		if (name.equals("thaumcraft.common.lib.research.ScanManager")) {
			...
			for (MethodNode m : classNode.methods) {
				if (m.name.equals("generateItemHash")) { // First most costly thing
					...
					m.instructions.insertBefore(first, new MethodInsnNode(Opcodes.INVOKESTATIC, "gregtech6/asm/transformers/Thaumcraft_AspectLagFix", "getCachedItemHash", "(Lnet/minecraft/item/Item;I)I", false));
					...
					for (AbstractInsnNode node = first.getNext(); node != null; node = node.getNext()) {
						if (node instanceof InsnNode) {
							...
							if (ret.getOpcode() == Opcodes.IRETURN) {
								...
								m.instructions.insertBefore(ret, new MethodInsnNode(Opcodes.INVOKESTATIC, "gregtech6/asm/transformers/Thaumcraft_AspectLagFix", "setCachedItemHash", "(ILnet/minecraft/item/Item;I)I", false));
							}
						}
					}
				}
		...
		if (name.equals("thaumcraft.common.lib.crafting.ThaumcraftCraftingManager")) {
			...
			for (MethodNode m : classNode.methods) {
				if (m.name.equals("getObjectTags")) { // Second most costly thing
					...
					LabelNode first = (LabelNode) m.instructions.get(0);
					m.instructions.insertBefore(first, new VarInsnNode(Opcodes.ALOAD, 0)); // Load item
					m.instructions.insertBefore(first, new MethodInsnNode(Opcodes.INVOKESTATIC, "gregtech6/asm/transformers/Thaumcraft_AspectLagFix", "getCachedAspectTags", "(Lnet/minecraft/item/ItemStack;)Lthaumcraft/api/aspects/AspectList;", false));
					m.instructions.insertBefore(first, new InsnNode(Opcodes.DUP)); // duplicate return
					...
					for (AbstractInsnNode node = first.getNext(); node != null; node = node.getNext()) {
						if (node instanceof InsnNode) {
							...
							if (ret.getOpcode() == Opcodes.ARETURN) {
								m.instructions.insertBefore(ret, new VarInsnNode(Opcodes.ALOAD, 0)); // Load item, hash is already on the stack
								m.instructions.insertBefore(ret, new MethodInsnNode(Opcodes.INVOKESTATIC, "gregtech6/asm/transformers/Thaumcraft_AspectLagFix", "setCachedAspectTags", "(Lthaumcraft/api/aspects/AspectList;Lnet/minecraft/item/ItemStack;)Lthaumcraft/api/aspects/AspectList;", false));
							}
						}
					}
				}
