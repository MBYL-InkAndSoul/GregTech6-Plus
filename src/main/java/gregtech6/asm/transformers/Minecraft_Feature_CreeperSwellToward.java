	...
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		...
		for (MethodNode m: classNode.methods) {
			if (m.name.equals("shouldExecute") || (m.name.equals("a") && m.desc.equals("()Z"))) {
				...
				m.instructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "gregtech6/asm/transformers/minecraft/Replacements", "EntityAICreeperSwell_shouldExecute", "(Lnet/minecraft/entity/monster/EntityCreeper;)Z", false));
